...
# Non-assigned genes (9) will be introns. CDS (8) will be replaced with 0/4 later.
init <- Sys.time()
features <- c("gene"=9L,"CDS"=8L,"three_prime_UTR"=3L,"five_prime_UTR"=5L)
for (n in 1:length(features)) {
  feature <- gff.table[gff.table$feature == names(features[n]),]
  symbol <- features[n]
  for (i in 1:nrow(feature)) {
    gffseq[feature[i,3]:feature[i,4]] <- symbol
  }
}
Sys.time()-init
