...
#setwd("F:/Internship/2_GenomicsData/TestPopGenome")
...
if (MK==TRUE) {
  ...
  region <- set.synnonsyn(region,ref.chr=sprintf("chr%s.fa",chrom),save.codons=TRUE) 
...
# With save.codons=TRUE, PopGenome stores codon data in:
# get.codons: 
# @codons: changes from 
...
mafan <- merge(freqs.df,aarefs,all.x=TRUE) # Intersection between SNPs in the biallelic matrix and the AA in the VCF file
...
measures <- function(object) {
  ...
  for (window in 1:nrow(windows)) {
    ...
    # The 'winsites' variable contains the positions of the biallelic variants in each window. 
    # By using this information, we can subset the DAF of the sites in this window contained in 'mafan'.
    winDAF <- mafan[mafan$POS %in% winsites[[window]],5]
    ...
    synmask <- slide@region.data@synonymous[[window]][winsites[[window]] %in% mafan$POS]
    ...
    # 1) Use four-fold degenerated sitesb2) Remove sites according to DAF
    
    # STANDARD

    syn <- change@synonymous[[1]] # Contains syn and non-syn positions
    Ps <- sum(polym[syn==4],na.rm=TRUE)
    Pn <- sum(polym[syn==0],na.rm=TRUE)
    Ds <- sum(divtotal[syn==4],na.rm=TRUE)
    Dn <- sum(divtotal[syn==0],na.rm=TRUE)
    NI <- (Pn/Ps)/(Dn/Ds)
    alpha <- 1-NI
    
    region@region.data@CodingSNPS #Coding positions (TRUE/FALSE)
