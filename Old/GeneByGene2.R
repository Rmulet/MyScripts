...
overlap <- countOverlaps(query=grgenes,subject=grmask) # Determine what genes overlap with the mask
...
for(i in 1:length(grgenes)) {
  ...
  if (sum(width(inter)) < 0.5*width(grgenes[i])) { # Threshold for the number of accessible positions
    print (sum(width(inter)))
    next # If below the threshold, the gene is skipped
  }
  ...
  r2 <- splitting.data(region,positions=y,type=2)
  print(region@n.sites)
}
Sys.time()-init
...
for(i in 1:nrow(genes)) {
  ini <- genes[i,3]; end <- genes[i,4]
  mask.local <- strsplit(as.character(subseq(f,start=ini,end=end)),"")[[1]]
  pass <- mask.local == "P"
  vec.pass <- (ini:end)[pass] # Vector with gene positions that are accessible
  if (sum(pass) < 50) { next }# Threshold of positions that pass the filter
  # filename <- merge.vcf(ini,end)
