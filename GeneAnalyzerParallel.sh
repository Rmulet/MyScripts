...
display_usage() { 
	...
	echo -e " -t, --threads \t Number of chromosomes analysed in parallel [5]"
	} 

exec > >(tee -a GeneAnalyzer.log) 2>&1
...
export POP="FALSE"
export DL="FALSE"
export CHR=`seq 1 22` # All autosomal chromosomes
...
export WORKING="$HOME/Genomics" # MODIFY THIS VARIABLE TO EXECUTE IN A DIFFERENT DIRECTORY
...
genome_analysis() {
	...
	mkdir -p chr$i
	cp -n $finaldir/'GenesTable.RData' $finaldir/chr$i
	ln -s $finaldir/gffseq_chr$i.RData $finaldir/chr$i

	finaldir=${finaldir}/chr$i
	# POLYMORPHISM #
	...
	if [ "$POP" != "FALSE" ]; then # Note that "PopulationIndividualsList.panel" is assumed to contain individuals and populations
		...
		bcftools view -Oz --force-samples -s $popins chr$i\_gp.vcf.gz > chr$i$2.vcf.gz # Some have been removed because they are inbred (force-samples to skip)
		gpfile=chr$i$2.vcf.gz # In population mode, then gpfile is the 
		echo $gpfile	
		tabix -p vcf $gpfile
	fi
...
#		parallel --gnu -j8 genome_analysis ::: $CHR ::: $spop
	...
	#	parallel --gnu -j8 genome_analysis ::: $CHR ::: $npop
	echo $CHR | xargs -n 1 -P$THREADS bash -c 'genome_analysis "$@" $npop' _
	done	
else
#	parallel --gnu -j8 genome_analysis ::: $CHR ::: $POP
