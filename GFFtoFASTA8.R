...
# regions, it applies check_fold to determite their degeneracy (4 for 4-fold, 0 for 0-fold)
...
mRNA <- transcripts(txdb) # The function genes() returns HUGO genes, not UCSC transcripts
CDS <- cds(txdb)
exons <- exons(txdb)
...
features <- c("mRNA"=9L,"exons"=8L,"CDS"=7L,"threeUTR"=3L,"fiveUTR"=5L)
...
# Note that the difference between 'mRNA' and 'cdsBy' is that the latter only returns
# transcripts which encode proteins, i.e. not non-coding sequences.

codingRNA <- cdsBy(txdb,"tx")
komplement <- c(4,3,2,1,5) # For regions in the reverse strand: opposite of T=1,C=2,G=3,A=4 (vectorisation)
init <- Sys.time()
for (x in 1:length(codingRNA)) { # x for each gene/transcript in the GFF file
  tab <- data.frame(start=start(codingRNA[[x]]),end=end(codingRNA[[x]]),strand=strand(codingRNA[[x]]))
