...
get_gff_info(region,gffpath,22,c(ini,end),feature="gene") -> Positions of genes, exons...
...
##########################################
## ARGUMENT PARSING AND SCRIPT LOCATION ##
##########################################
...
ncolstest <- function() { # Based on this function, the best ncols is 8000-10000
test <- vector()
  for (i in c(100,1000,seq(2000,15000,by=1000))) {
    ...
    ncolstest <- c(test,mean(c(t1,t2,t3)))
  }
}
...
if (MK==TRUE) {
  region <- readVCF(filename,numcols=9000,tid=chrom,from=ini,to=end,include.unknown=TRUE,gffpath=sprintf("chr%s.gff",chrom))
...
windows2 <- data.frame(rep("chr22",nwin),start=windows[,1],end=windows[,2])
grwindows <- with(windosws2,GRanges(chr,IRanges(start,end)))
...
measures <- function(object) {
  ...
  system.time(mclapply(X=1:nwin,FUN=measinternal,mc.cores=2))
  system.time(lapply(X=1:nwin,FUN=measinternal))
  return(tabsum)  
}
