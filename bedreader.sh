...
do
	...
	echo $chrom
	if [ "$4" == "-c" ] || [ "$4" == "-cnvs" ]; then
		tabix -h $2 $chrom:$pos1-$pos2 | grep -v "<CN" > $gpfile.$k.vcf # We remove CNVs
		tabix -h $3 $chrom:$pos1-$pos2 > $alnfile.$k.vcf # We remove CNVs
	else	
		tabix -h $2 $chrom:$pos1-$pos2 > $gpfile.$k.vcf	
		tabix -h $3 $chrom:$pos1-$pos2 > $alnfile.$k.vcf 
	fi
