...
notac <- as.numeric(args[9]) # Number of non-accessible positions
...
if (is.null(region)||is.logical(region)||region@n.biallelic.sites==0) { # If readVCF fails, region=FALSE. If no variants, region=NULL
  ...
  nwin=floor((end+1-ini)/wsize)
  windows <- cbind(start=seq(ini,end-wsize+1,by=wsize),end=seq(ini+wsize,end+1,by=wsize))
  newrows <- matrix(rep(c(0,0,0,0,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,0,0,NA,0,0,0,0,0,0,0,0,NA,NA,NA,NA,0,0,0,0,0,0,0,NA,NA,0,0,notac),nwin),nrow=nwin,byrow=TRUE)
  colnames(newrows) <- c("S","Pi","theta","S2","Tajima_D","FuLi_F","FuLi_D","FayWu_H","Zeng_E",
	...
	"Psel.neutral","Psel.weak","alpha.cor","Fisher2","msel","mneu","notac")
...
windows <- data.frame(start=ini,end=end)
nwin = 1 ; window = 1
...
n_sites <- region@n.sites
n_biallelic_sites <- region@n.biallelic.sites
n_gaps <- region@n.gaps
n_unknowns <- region@n.unknowns
n_valid_sites <- region@n.valid.sites
n_polyallelic_sites <- region@n.polyallelic.sites
trans_transv_ratio <- region@trans.transv.ratio
################
...
measures <- function(object) {
  ...
  tabsum <- as.data.frame(matrix(numeric(nwin*27),ncol=27,nrow=nwin))
  ...
  bial <- get.biallelic.matrix(object,1) # Biallelic matrix of the region
  ...
  winstart <- ini # Since we have only 1 window, winstart is the same as ini
  # Total number of sites: 1) Remove NA in humans 2) Remove polyallelic sites 3) Remove non-accessible regions
  multisites <- region@n.polyallelic.sites # N polyallelic sites in that window
  m <- wsize-sum(misshuman,na.rm=T)-multisites-notac
  ...
  if (!is.null(mafan$DAF)) {
    DAF <- hist(as.vector(na.omit(mafan$DAF)),seq(0.0,1,0.05),plot=F)$counts
  ...
  winsites <- region@region.data@biallelic.sites[[1]] # Biallelic sites in this window
  ...
  alpha.cor,test2,m.sel,m.neu,notac)
  tabsum[window,] <- newrow
...
#region <- calc.R2(region)
#region@region.stats@linkage.disequilibrium   # no sé, surt un núm només...

##### Aquest sí que sembla funcionar bé...
region <- linkage.stats(region,detail=FALSE)
#get.linkage(region)[[1]]
...
#region@Wall.B[,1]     # cal dividir també per la wsize???
Wall_B <- round(region@Wall.B[,1],7)
Wall_Q <- round(region@Wall.Q[,1],7)
Rozas_ZA <- round(region@Rozas.ZA[,1],7)
Rozas_ZZ <- round(region@Rozas.ZZ[,1],7)
Kelly_ZnS <- round(region@Kelly.Z_nS[,1],7)

region <- F_ST.stats(region,detail=FALSE)
# region@region.stats@haplotype.diversity
# o accedir al valor com
#region@hap.diversity.within[,1]    # cal dividir també per la wsize???
#region@nucleotide.F_ST[,1]
#region@haplotype.F_ST[,1]
...
nucleotide_FST <- round(region@nucleotide.F_ST[,1],7)
haplotype_FST <- round(region@haplotype.F_ST[,1],7)
Nei_GST <- round(region@Nei.G_ST[,1],7)
Hudson_GST <- round(region@Hudson.G_ST[,1],7)
Hudson_HST <- round(region@Hudson.H_ST[,1],7)
Hudson_KST <- round(region@Hudson.K_ST[,1],7)
nuc_diversity_within <- round(region@nuc.diversity.within[,1]/wsize,7)
hap_diversity_within <- round(region@hap.diversity.within[,1],7)
...
regiondata <- measures(region)
S2 <- region@n.segregating.sites[,1] # Segretaging sites excluding unknowns
Tajima_D <- round(region@Tajima.D[,1]/wsize,7)
FuLi_F <- round(region@Fu.Li.F[,1]/wsize,7)
FuLi_D <- round(region@Fu.Li.D[,1]/wsize,7)
FayWu_H <- round(region@Fay.Wu.H[,1]/wsize,7)
Zeng_E <- round(region@Zeng.E[,1]/wsize,7)
#Rozas_R2 <- round(slide@Rozas.R_2[,1]/wsize,7) # it is a detailed statistic; if wanted, apply option: do.R2
theta <- round(region@theta_Watterson[,1]/wsize,7)
